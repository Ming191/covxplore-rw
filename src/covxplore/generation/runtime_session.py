...
from covxplore.types import TestResult, TestSuite, UnvisitedBranch, UnvisitedStatement
...
class TestSuiteCodec:
    ...
    def _dump_coverage(coverage: CoverageState) -> dict:
        return {
            ...
            "_best_statement_visited": coverage._best_statement_visited,
            "_best_branch_visited": coverage._best_branch_visited,
            "_structural_totals_known": coverage._structural_totals_known,
        }

    def _load_coverage(self, data: Mapping[str, Any]) -> CoverageState:
        coverage = CoverageState()
        coverage.consecutive_redundant = self._non_negative_int(
            data.get("consecutive_redundant"), "consecutive_redundant"
        )
        stmt_ids = data.get("_cumulative_uncovered_stmt_ids")
        coverage._cumulative_uncovered_stmt_ids = (
            None if stmt_ids is None else {int(node_id) for node_id in stmt_ids}
        )
        branch_keys = data.get("_cumulative_uncovered_branch_keys")
        coverage._cumulative_uncovered_branch_keys = (
            None
        ...
        for field_name in (
            ...
            setattr(
                coverage,
                field_name,
                self._non_negative_int(data.get(field_name), field_name),
            )
